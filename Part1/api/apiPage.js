...
const createSearchRepos = () => {
  ...
  input.placeholder = 'Search for Users Repositiries'
...
const getRepos = (reposUrl) => {
  ...
  fetch(reposUrl)
    ...
    .then(json => {
      return json.status == 200 ? json.json() : Promise.reject('ошибка')
    })
    .then(searchRepos)
    .catch(console.log)
...
const getUser = (login) => {
  ...
  fetch(url + login)
    .then((json) => {
      return json.status == 200 ? json.json() : Promise.reject('a;lsdla;')
    })
    .then(createNewUser)
    .catch(console.log)
